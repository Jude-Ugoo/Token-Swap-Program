...
describe("swap", async () => {
  ...
  const tokenBOfferedAmount = new BN(1_000_000);

  before(
    "Creates Aliice and Bob accounts, 2 token mints, and associated token accounts for both tokens for both users",
    async () => {
      ...
      accounts.makerTockenAccountB = aliceTokenAccountB;
...
function makeKeypairs(arg0: number): [anchor.web3.Keypair, anchor.web3.Keypair, anchor.web3.Keypair, anchor.web3.Keypair] {
